...
```bash
git clone https://github.com/your-username/keyspace.git
cd keyspace
```
...
Hoặc sử dụng SQLite (đơn giản hơn):

```env
DB_CONNECTION=sqlite
# Comment các dòng DB_HOST, DB_PORT, DB_DATABASE, DB_USERNAME, DB_PASSWORD
```
...
# Hoặc sử dụng script tích hợp sẵn (chạy cùng lúc server, queue, logs, vite)
composer dev
```
